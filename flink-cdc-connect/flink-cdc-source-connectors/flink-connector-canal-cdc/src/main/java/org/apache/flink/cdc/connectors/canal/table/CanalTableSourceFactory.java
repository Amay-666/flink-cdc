...
/** Factory for creating configured instance of {@link CanalTableSource}. */
public class CanalTableSourceFactory implements DynamicTableSourceFactory {

    private static final Logger LOG = LoggerFactory.getLogger(CanalTableSourceFactory.class);

    private static final String IDENTIFIER = "canal-cdc";
    ...
    public DynamicTableSource createDynamicTableSource(Context context) {
        ...
        int port = config.get(CanalSourceOptions.CANAL_MYSQL_PORT);
        ...
        String kafkaBootstrapServers = config.get(CanalSourceOptions.KAFKA_BOOTSTRAP_SERVERS);
        String kafkaGroupId = config.getOptional(CanalSourceOptions.KAFKA_GROUP_ID).orElse(null);
        List<String> kafkaTopics = getKafkaTopics(config.get(CanalSourceOptions.SCAN_KAFKA_TOPICS));
        Map<String, String> options = context.getCatalogTable().getOptions();
        CanalSourceOptions.MessageFormat messageFormat =
                getEnumOption(
                        options,
                        CanalSourceOptions.MESSAGE_FORMAT,
                        CanalSourceOptions.MessageFormat.class);
        CanalSourceOptions.EventTime eventTime =
                getEnumOption(
                        options, CanalSourceOptions.EVENT_TIME, CanalSourceOptions.EventTime.class);
        CanalSourceOptions.BoundaryMode boundaryMode =
                getEnumOption(
                        options,
                        CanalSourceOptions.BOUNDARY_MODE,
                        CanalSourceOptions.BoundaryMode.class);
        ...
        CanalSourceOptions.DdlParser ddlParser =
                getEnumOption(
                        options,
                        CanalSourceOptions.CANAL_DDL_PARSER,
                        CanalSourceOptions.DdlParser.class);
    ...
    public Set<ConfigOption<?>> requiredOptions() {
        ...
        options.add(CanalSourceOptions.KAFKA_BOOTSTRAP_SERVERS);
        options.add(CanalSourceOptions.SCAN_KAFKA_TOPICS);
        return options;
    }
    ...
    public Set<ConfigOption<?>> optionalOptions() {
        Set<ConfigOption<?>> options = new HashSet<>();
        options.add(CanalSourceOptions.CANAL_MYSQL_PORT);
        options.add(CanalSourceOptions.SERVER_TIME_ZONE);
        ...
        options.add(CanalSourceOptions.KAFKA_GROUP_ID);
        options.add(CanalSourceOptions.MESSAGE_FORMAT);
        options.add(CanalSourceOptions.EVENT_TIME);
        options.add(CanalSourceOptions.BOUNDARY_MODE);
        options.add(CanalSourceOptions.KAFKA_STARTUP_MODE);
        options.add(CanalSourceOptions.CANAL_DDL_PARSER);
        return options;
    }
                ...
                (key, value) -> {
                    if (key.startsWith(CanalSourceOptions.KAFKA_PROPERTIES_PREFIX)) {
                        kafkaProperties.setProperty(
                                key.substring(CanalSourceOptions.KAFKA_PROPERTIES_PREFIX.length()),
                                value);
                    }
                });
    ...
    private static ZoneId getServerTimeZone(ReadableConfig config) {
        final String serverTimeZone = config.get(CanalSourceOptions.SERVER_TIME_ZONE);
        if (serverTimeZone != null) {
            return ZoneId.of(serverTimeZone);
        } else {
            LOG.warn(
                    "{} is not set, which might cause data inconsistencies for time-related fields.",
                    CanalSourceOptions.SERVER_TIME_ZONE.key());
