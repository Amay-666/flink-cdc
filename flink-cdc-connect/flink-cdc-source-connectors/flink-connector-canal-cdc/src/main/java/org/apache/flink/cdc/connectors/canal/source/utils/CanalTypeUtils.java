...
/**
 * A utility class for converting MySQL types to Flink types.
 *
 * <p>The type name is read from the JDBC metadata {@code TYPE_NAME} column. Different MySQL JDBC
 * drivers may return a plain type name (e.g. {@code INT}) or one with modifiers (e.g. {@code INT
 * UNSIGNED ZEROFILL}), so the type name is normalized to {@code <BASE TYPE>[ UNSIGNED][ ZEROFILL]}
 * before mapping.
 */
public class CanalTypeUtils {
