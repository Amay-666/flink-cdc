...
import org.apache.flink.cdc.connectors.canal.source.handler.CanalSchemaChangeHandler;
 ...
 * {@link CanalOffsetSupplier} computed during the snapshot phase, so no change is skipped at the
 * full-&gt;incremental switch.
 *
 * <p>When the split is a bounded read (its ending offset is a real event time rather than {@link
 * CanalOffset#NO_STOPPING_OFFSET}) the task dispatches the {@link WatermarkKind#END} watermark once
 * it has consumed past the ending offset, which finalizes the incremental snapshot of the split.
 *
 * <p>The position of the most recent consumed message is kept as {@link CanalOffset} and committed
 * by {@link CanalDialect#notifyCheckpointComplete(long, Offset)}; the Kafka group offset itself is
 * left untouched because exactly-once recovery relies on the Flink checkpointed state.
 */
public class CanalStreamFetchTask implements FetchTask<SourceSplitBase> {

    private static final Logger LOG = LoggerFactory.getLogger(CanalStreamFetchTask.class);
    ...
    private final CanalOffset startingOffset;
    private final CanalOffset endingOffset;

    private volatile boolean taskRunning = false;
    private volatile boolean stopped = false;

    /** The Kafka consumer, owned by the {@link CanalSourceFetchTaskContext}. */
    private volatile KafkaConsumer<String, String> consumer;

    /** The position of the last consumed message. */
    private volatile CanalOffset currentOffset;

    private volatile CanalOffset lastCommittedOffset;

    /** Applies the DDL messages to the shared schema; built once per execute. */
    private volatile CanalSchemaChangeHandler schemaChangeHandler;

    public CanalStreamFetchTask(StreamSplit split) {
        this.split = split;
        this.startingOffset =
                split.getStartingOffset() == null
                        ? CanalOffset.INITIAL_OFFSET
                        : (CanalOffset) split.getStartingOffset();
        this.endingOffset =
                split.getEndingOffset() == null
                        ? CanalOffset.NO_STOPPING_OFFSET
                        : (CanalOffset) split.getEndingOffset();
    ...
    public void execute(FetchTask.Context context) throws Exception {
        ...
        if (!(context instanceof CanalSourceFetchTaskContext)) {
        ...
        CanalSourceConfig sourceConfig = sourceFetchContext.getSourceConfig();
        this.schemaChangeHandler = new CanalSchemaChangeHandler(sourceConfig);
        taskRunning = true;
        try {
    ...
    private boolean processRecords(
            CanalSourceFetchTaskContext sourceFetchContext, ConsumerRecords<String, String> records)
            throws Exception {
        CanalOffset lastOffset = null;
        for (ConsumerRecord<String, String> record : records) {
            if (stopped) {
                break;
            }
            CanalFlatMessage message = CanalFlatMessageParser.parse(record.value());
            if (message == null) {
                LOG.warn(
            ...
            lastOffset =
                    new CanalOffset(
                            CanalRecordConverter.eventTime(
    ...
    private void handleDdlMessage(
            CanalSourceFetchTaskContext sourceFetchContext,
            CanalFlatMessage message,
            CanalOffset offset)
            throws IOException, InterruptedException {
        CanalSchemaChangeHandler handler = schemaChangeHandler;
        if (handler == null) {
    ...
    private void assignAndSeek(KafkaConsumer<String, String> consumer, CanalSourceConfig sourceConfig) {
    ...
    private void dispatchEndWatermark(CanalSourceFetchTaskContext sourceFetchContext)
            throws Exception {
    ...
    private boolean isBoundedRead() {
        return endingOffset.getEventTime() != CanalOffset.NO_STOPPING_OFFSET.getEventTime();
    }
    ...
    public void commitCurrentOffset(Offset offset) {
        if (offset instanceof CanalOffset) {
            CanalOffset canalOffset = (CanalOffset) offset;
