 ...
 * {@link CanalRecordFactory} so that the snapshot rows and the streaming (canal) messages build
 ...
 * snapshot splitter.
 */
public class CanalSchema extends io.debezium.relational.RelationalDatabaseSchema {
    ...
    public CanalSchema(CanalSourceConfig sourceConfig) {
        this(sourceConfig, new CanalRecordFactory(sourceConfig));
    }

    /** Creates a schema that shares the given record factory (used by the fetch task context). */
    public CanalSchema(CanalSourceConfig sourceConfig, CanalRecordFactory recordFactory) {
        super(
