 ...
 * CanalSchema} — e.g. a table created after the snapshot discovery, or a stream-only setup without
 * JDBC credentials — the column types are reconstructed from the message itself. The canonical path
 * still prefers the JDBC schema from {@code CanalSchema}.
 */
public class CanalTableUtils {
     ...
     *     all
     */
    public static Table buildTable(CanalFlatMessage message) {
        TableId tableId =
    ...
    private static List<String> columnNames(CanalFlatMessage message) {
     ...
     *
     * <p>Shared with the DDL parsers ({@code CanalDruidDdlParser}), which reconstruct the type
