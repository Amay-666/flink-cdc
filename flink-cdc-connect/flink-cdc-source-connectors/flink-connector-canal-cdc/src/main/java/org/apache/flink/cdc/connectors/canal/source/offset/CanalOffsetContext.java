 ...
 *
 * <p>The offset is a {@link CanalOffset} — the Kafka position of the last consumed message, keyed
 ...
 */
public class CanalOffsetContext implements OffsetContext {

    private final CanalSourceInfoStructMaker sourceInfoStructMaker;
    private CanalOffset offset = CanalOffset.INITIAL_OFFSET;
    private CanalSourceInfo sourceInfo;

    public CanalOffsetContext(MySqlConnectorConfig config) {
        this.sourceInfoStructMaker =
                new CanalSourceInfoStructMaker("canal", CanalSourceInfoStructMaker.DEBEZIUM_VERSION, config);
    }

    /** Updates the current offset. */
    public void updateOffset(CanalOffset offset) {
        this.offset = offset;
    }

    public CanalOffset getCanalOffset() {
        return offset;
    }

    /** Sets the source info used for the {@code source} struct of the offset. */
    public void setSourceInfo(CanalSourceInfo sourceInfo) {
