...
/** The source builder for {@link CanalSource}. */
@Experimental
public class CanalSourceBuilder<T> {

    private final CanalSourceConfigFactory configFactory = new CanalSourceConfigFactory();
    private DebeziumDeserializationSchema<T> deserializer;

    private CanalSourceBuilder() {}

    /** The hostname of the MySQL database to monitor for changes. */
    public CanalSourceBuilder<T> hostname(String hostname) {
        this.configFactory.hostname(hostname);
        return this;
    }

    /** Integer port number of the MySQL database server. */
    public CanalSourceBuilder<T> port(int port) {
        this.configFactory.port(port);
        return this;
    }

    /** The name of the MySQL database from which to read the snapshot. */
    public CanalSourceBuilder<T> username(String username) {
        this.configFactory.username(username);
        return this;
    }

    /** Password to use when connecting to the MySQL database server. */
    public CanalSourceBuilder<T> password(String password) {
     ...
     */
    public CanalSourceBuilder<T> serverTimeZone(String timeZone) {
    ...
    public CanalSourceBuilder<T> databaseList(String... databaseList) {
    ...
    public CanalSourceBuilder<T> closeIdleReaders(boolean closeIdleReaders) {
        this.configFactory.closeIdleReaders(closeIdleReaders);
        return this;
    }

    /** The connection pool size. */
    public CanalSourceBuilder<T> connectionPoolSize(int connectionPoolSize) {
    ...
    public CanalSourceBuilder<T> connectTimeout(Duration connectTimeout) {
        this.configFactory.connectTimeout(connectTimeout);
        return this;
    }

    /** The max retry times to get connection. */
    public CanalSourceBuilder<T> connectMaxRetries(int connectMaxRetries) {
    ...
    public CanalSourceBuilder<T> debeziumProperties(Properties debeziumProperties) {
        this.configFactory.debeziumProperties(debeziumProperties);
        return this;
    }

    /** The deserializer used to convert from consumed {@code SourceRecord}. */
    public CanalSourceBuilder<T> deserializer(DebeziumDeserializationSchema<T> deserializer) {
        this.deserializer = deserializer;
        return this;
    }

    /**
     * Build the {@link CanalSource}.
     *
     * <p>Wires the {@link CanalOffsetFactory} and the {@link CanalDialect} into the incremental
     ...
     *
     * @return a CanalSource with the settings made for this builder.
     */
    public CanalSource<T> build() {
        CanalOffsetFactory offsetFactory = new CanalOffsetFactory();
        CanalDialect dialect = new CanalDialect(configFactory.create(0));
        return new CanalSource<>(
                configFactory, checkNotNull(deserializer), offsetFactory, dialect);
    }

    public static <T> CanalSourceBuilder<T> builder() {
        return new CanalSourceBuilder<>();
    }
}
