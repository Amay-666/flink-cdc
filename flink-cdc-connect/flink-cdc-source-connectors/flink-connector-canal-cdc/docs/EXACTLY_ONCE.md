...
本文档描述 flink-connector-canal-cdc 如何基于 flink-cdc-base 的增量快照框架（FLIP-27 +
...
| event time | 水位比较使用的事件时间，由 `scan.canal.event-time` 决定（默认 `ES`，可配 `TS`） | 消息内容 |

天然时序关系：`es ≤ ts ≤ record timestamp(发送时刻)`。

`CanalOffset = (eventTime, partition, offset)`，排序规则 `eventTime → partition → offset`。
...
1. **LOW**：JDBC 读分片数据**之前**，通过 `CanalDialect.displayCurrentOffset` 捕获当前
   Kafka 流位置（每个分区读最新一条消息得到 `(eventTime, partition, offset)`）。
2. **快照读**：`CanalScanFetchTask.CanalSnapshotSplitReadTask` 按分片 SQL 读取 MySQL 行，
   转成 `READ` 记录入队。
...
`minOffset` 是**跨分区**的最小值：`CanalOffset` 按 `eventTime → partition → offset` 排序取
...
主 stream 启动时 `CanalStreamFetchTask.assignAndSeek`：
...
设起始水位为 `T`（即起始 `CanalOffset.eventTime`）。定位点 `L = offsetsForTimes(T)` 之前的
...
| 增量 offset 与 checkpoint 对齐 | `CanalDialect.notifyCheckpointComplete` → `CanalStreamFetchTask.commitCurrentOffset`，仅在 checkpoint 完成后提交 | `CanalDialect.java` |
...
| 配置 | 作用 |
|---|---|
| `scan.canal.event-time` | 水位事件时间来源：`ES`（默认）/ `TS` |
