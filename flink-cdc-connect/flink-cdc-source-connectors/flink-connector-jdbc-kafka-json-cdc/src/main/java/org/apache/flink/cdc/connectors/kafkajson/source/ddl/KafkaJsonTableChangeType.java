 ...
 * express. The canal connector uses it to announce a table/column rename or truncate to a
 * downstream that builds its own event handling (the released flink-cdc runtime has no {@code
 * RENAME_TABLE} event, so a table rename would otherwise be flattened into a {@code DROP}+{@code
 * CREATE} pair; likewise, a truncate would be indistinguishable from a {@code DROP}+{@code CREATE}
 * pair).
 *
 * <p>For column-level changes, the specific change details are recorded in a {@link
 * ColumnChangeInfo} list so that downstream handlers can react differently to each kind of schema
 * evolution (e.g., type change vs. comment change). The Debezium ANTLR parser ({@link
