...
/**
 * Basic class read {@link SourceSplitBase} and return {@link SourceRecord}.<br>
 * Copied from IncrementalSourceSplitReader
 */
@Experimental
public class KafkaJsonIncrementalSourceSplitReader<C extends SourceConfig>
        implements SplitReader<SourceRecords, SourceSplitBase> {

    private static final Logger LOG =
            LoggerFactory.getLogger(KafkaJsonIncrementalSourceSplitReader.class);
    ...
    @Nullable private KafkaJsonIncrementalSourceStreamFetcher reusedStreamFetcher;
    ...
    public KafkaJsonIncrementalSourceSplitReader(
            int subtaskId,
    ...
    private void suspendStreamReaderIfNeed() throws Exception {
        if (currentFetcher != null
                && currentFetcher instanceof KafkaJsonIncrementalSourceStreamFetcher
                && context.isStreamSplitReaderSuspended()
                && !currentFetcher.isFinished()) {
            ((KafkaJsonIncrementalSourceStreamFetcher) currentFetcher).stopReadTask();
    ...
    private ChangeEventRecords pollSplitRecords() throws InterruptedException {
        ...
        } else if (currentFetcher instanceof KafkaJsonIncrementalSourceStreamFetcher) {
