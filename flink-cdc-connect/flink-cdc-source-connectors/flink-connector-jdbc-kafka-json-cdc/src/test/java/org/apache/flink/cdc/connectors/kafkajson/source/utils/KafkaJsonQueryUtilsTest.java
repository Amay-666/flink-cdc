...
/** Unit test for the SQL generation of {@link KafkaJsonQueryUtils}. */
class KafkaJsonQueryUtilsTest {
    ...
    void testQuoteTableId() {
        assertEquals("`test`.`users`", KafkaJsonQueryUtils.quote(TABLE_ID));
    }

    @Test
    void testQuoteColumnName() {
        assertEquals("`id`", KafkaJsonQueryUtils.quote("id"));
    }

    @Test
    void testBuildSplitScanQueryOnlySplit() {
        String sql = KafkaJsonQueryUtils.buildSplitScanQuery(TABLE_ID, singleKey(), true, true);
    ...
    void testBuildSplitScanQueryLastSplit() {
        String sql = KafkaJsonQueryUtils.buildSplitScanQuery(TABLE_ID, singleKey(), false, true);
        assertEquals("SELECT * FROM `test`.`users` WHERE id >= ?", sql);
    }

    @Test
    void testBuildSplitScanQueryMiddleSplit() {
        String sql = KafkaJsonQueryUtils.buildSplitScanQuery(TABLE_ID, singleKey(), false, false);
        assertEquals(
